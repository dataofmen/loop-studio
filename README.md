...
3. **처음 한 번만 승인이 필요합니다.** 더블클릭하면 "열 수 없습니다"가 나옵니다 —
   `시스템 설정` → `개인정보 보호 및 보안`에서 아래로 스크롤해 **"확인 없이 열기"**를
   누르세요(우클릭 → **열기**도 됩니다). 이후로는 더블클릭으로 열립니다.

   Apple 유료 개발자 프로그램(연 $99)에 가입해 서명·공증하지 않으면 이 절차를 없앨 수 없습니다.
