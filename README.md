...
이 앱은 Apple 유료 개발자 인증서로 서명·공증(notarization)되지 않았습니다. 그래서 macOS가
첫 실행을 한 번 막습니다. **한 번만** 넘어가면 그 뒤로는 보통 앱처럼 더블클릭으로 열립니다.

> 🚨 **첫 경고창에서 Return(엔터) 키를 누르지 마세요.**
> 기본 버튼이 **`휴지통으로 이동`** 이라서, 엔터를 치면 앱이 그대로 삭제됩니다.
> **`완료`(Done)** 를 마우스로 누르세요.

또 하나 — **`확인 없이 열기` 버튼은 처음부터 있지 않습니다. 앱을 한 번 열어 보고 차단된
뒤에야 설정에 나타납니다.** 순서를 지켜 주세요.

1. `응용 프로그램`(Applications)에서 **Loop Studio를 더블클릭**합니다.
2. **"Loop Studio.app"을(를) 열지 못했습니다** — *Apple은 "Loop Studio.app"에 Mac을 손상시키거나
   개인 정보를 침해할 수 있는 멀웨어가 없는지 확인할 수 없습니다.* 라는 창이 뜹니다.
   버튼은 **`휴지통으로 이동`** 과 **`완료`** 둘뿐입니다 — **`완료`** 를 누르세요.
3. `시스템 설정`(  → 시스템 설정)을 열고 왼쪽에서 **개인정보 보호 및 보안**을 선택합니다.
4. 오른쪽을 **아래로 스크롤**하면 `보안` 항목에
   *"Loop Studio.app"이(가) 차단되었습니다…* 같은 문구와 함께 **`확인 없이 열기`** 버튼이 있습니다.
   이 버튼은 1~2단계를 거치지 않으면 보이지 않습니다.
5. 버튼을 누르고, 확인 창에서 한 번 더 **열기**를 누른 뒤 Touch ID나 로그인 암호로 승인합니다.
6. 앱이 열립니다. 이후에는 더블클릭으로 바로 열립니다.

### 터미널이 편하다면 (더 빠르고 확실합니다)

2~5단계를 한 줄로 대체할 수 있습니다. 경고창을 아예 만나지 않습니다.

```bash
xattr -d -r com.apple.quarantine "/Applications/Loop Studio.app"
```

이 명령은 "이 파일의 출처 확인을 건너뛴다"는 뜻입니다. 신뢰하는 파일에만 쓰세요.
받은 파일이 릴리스 페이지의 SHA-256과 일치하는지 먼저 확인하면 좋습니다:
`shasum -a 256 ~/Downloads/LoopStudio-*.dmg`

### 잘 안 될 때

| 증상 | 원인과 해결 |
|---|---|
| 엔터를 눌렀더니 앱이 사라졌다 | 기본 버튼이 `휴지통으로 이동`입니다. 휴지통에서 꺼내 `응용 프로그램`에 다시 넣고, 이번엔 `완료`를 누르세요. |
...
<details>
<summary>왜 이런 경고가 뜨나요?</summary>

Apple에 연간 비용을 내고 개발자 인증서를 받아 앱을 "공증"하면 이 경고가 사라집니다.
이 프로젝트는 그 비용을 쓰지 않기로 했기 때문에, 대신 위 절차로 한 번 승인해 주셔야 합니다.
앱 자체는 서명(ad-hoc)되어 있어 내용이 바뀌지 않았음은 macOS가 검증합니다 — 다만 "누가
만들었는지"를 Apple이 보증하지 않는 상태입니다.
</details>
